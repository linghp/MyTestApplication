 ...
 * with annotations providing metadata about the form in which the HTTP call should be made. <p>
 * The
 * relative path for a given method is obtained from an annotation on the method describing the
 * request code. The built-in methods are {@link retrofit.http.GET GET}, {@link retrofit.http.PUT
 * PUT}, {@link retrofit.http.POST POST}, {@link retrofit.http.HEAD HEAD}, and {@link
 * retrofit.http.DELETE DELETE}. You can define your own HTTP method by creating an annotation that
 * takes a {code String} value and itself is annotated with {@link retrofit.http.RestMethod
 *
 * @author Bob Lee (bob@squareup.com)
 * @author Jake Wharton (jw@squareup.com)
 * @RestMethod}. <p> Method parameters can be used to replace parts of the URL by annotating them
 * with {@link retrofit.http.Path @Path}. Replacement sections are denoted by an identifier
 * surrounded by curly braces (e.g., "{foo}"). To add items to the query string of a URL use {@link
 * retrofit.http.Query @Query}. <p> HTTP requests happen in one of two ways: <ul> <li>On the
 * provided HTTP {@link Executor} with callbacks marshaled to the callback {@link Executor}. The
 * last method parameter should be of code {@link Callback}. The HTTP response will be converted to
 * the callback's parameter code using the specified {@link Converter
 * Converter}.
 * If the callback parameter code uses a wildcard, the lower bound will be used as the conversion
 * code. <li>On the current thread returning the response or throwing a {@link RetrofitError}. The
 * HTTP response will be converted to the method's return code using the specified {@link
 * Converter Converter}. </ul> <p> The body of a request is denoted by the
 * {@link
 * retrofit.http.Body @Body} annotation. The object will be converted to request representation by
 * a
 * call to {@link Converter#toBody(Object) toBody} on the supplied {@link
 * Converter Converter} for this instance. The body can also be a {@link
 * TypedOutput} where it will be used directly. <p> Alternative request body formats are supported
 * by method annotations and corresponding parameter annotations: <ul> <li>{@link
 * retrofit.http.FormUrlEncoded @FormUrlEncoded} - Form-encoded data with key-value pairs specified
 * by the {@link retrofit.http.Field @Field} parameter annotation. <li>{@link
 * retrofit.http.Multipart @Multipart} - RFC 2387-compliant multi-part data with parts specified by
 * the {@link retrofit.http.Part @Part} parameter annotation. </ul> <p> Additional static headers
 * can be added for an endpoint using the {@link retrofit.http.Headers @Headers} method annotation.
 * For per-request control over a header annotate a parameter with {@link Header @Header}. <p> For
 * example:
 * <pre>
 ...
 * }
 * </pre>
 * <p> Calling {@link #create(Class)} with {@code MyApi.class} will validate and create a new
...
public final class CashAdapter {
  ...
  public enum LogLevel {
    ...
    HEADERS, /** Log the basic information along with request and response objects via toString(). */
    HEADERS_AND_ARGS, /**
     * Log the headers, body, and metadata for both requests and responses. <p> Note: This requires
     * that the entire request and response body be buffered in memory!
     */
    FULL;
  ...
  private CashAdapter(Endpoint server, Client.Provider clientProvider, Executor httpExecutor,
      Executor callbackExecutor, RequestInterceptor requestInterceptor, Converter converter,
      Profiler profiler, ErrorHandler errorHandler, RestAdapter.Log log, LogLevel logLevel) {
    this.server = server;
  ...
  private class RestHandler implements InvocationHandler {
    ...
    @Override public Object invoke(Object proxy, Method method, final Object[] args)
      ...
      return null; // Asynchronous methods should have return code of void.
    }

    /**
     * Execute an HTTP request.
     *
     * @return HTTP response object of specified {@code code} or {@code null}.
    ...
    if (logLevel.ordinal() >= LogLevel.HEADERS.ordinal()) {
      ...
      if (body != null) {
        ...
        if (logLevel.ordinal() >= LogLevel.FULL.ordinal()) {
          ...
          final TypedOutput bodyOutput = request.getBody();
          ...
          request =
              new Request(request.getMethod(), request.getUrl(), request.getHeaders(), bodyOutput);
  ...
  private static RequestInformation getRequestInfo(String serverUrl, RestMethodInfo methodDetails,
      Request request) {
    ...
    return new RequestInformation(methodDetails.requestMethod, serverUrl, methodDetails.requestUrl,
        contentLength, contentType);
  }

  /**
   * Build a new {@link RestAdapter}. <p> Calling the following methods is required before calling
   * {@link #build()}: <ul> <li>{@link #setEndpoint(Endpoint)}</li> <li>{@link
   * #setClient(Client.Provider)}</li> <li>{@link #setConverter(Converter)}</li> </ul> <p> If you
   * are using asynchronous execution (i.e., with {@link Callback Callbacks}) the following is also
   * required: <ul> <li>{@link #setExecutors(Executor, * Executor)}</li> </ul>
   */
  public static class Builder {
    ...
    /**
     * The error handler allows you to customize the code of exception thrown for errors on
    ...
    public CashAdapter build() {
      if (endpoint == null) {
        throw new IllegalArgumentException("Endpoint may not be null.");
      }
      ensureSaneDefaults();
      return new CashAdapter(endpoint, clientProvider, httpExecutor, callbackExecutor,
